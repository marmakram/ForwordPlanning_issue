    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for p1 in actionA.preconditions:
            for p2 in actionB.preconditions:
                if self.parent_layer.is_mutex(p1, p2):
                    return True
        return False
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        self.fill()
        for g in self.goal:
            costs_sum += self.LevelCost(self, g)
        return costs_sum


    def LevelCost(self, graph, goal):
        for idx, layer1 in enumerate(graph.literal_layers):
            if goal in layer1:
                    return idx
        return 0
    ...
    def h_maxlevel(self):
        ...
        self.fill()
        for g in self.goal:
            _costs_.append(self.LevelCost(self, g)) 
        return max(_costs_)
